...
const AuthScreen = () => {
  ...
  const [userType, setUserType] = useState('merchant'); // 'merchant' | 'delivery'
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Role Switcher - Clearly Separated */}
        <div className="px-8 mt-2 mb-6">
          <div className="grid grid-cols-2 p-1.5 bg-gray-50 rounded-2xl border border-gray-100 relative font-medium">
            {['merchant', 'delivery'].map((type) => (
               <button
                  key={type}
                  onClick={() => setUserType(type)}
                  className={`relative z-10 flex items-center justify-center gap-2 py-3 text-sm font-bold rounded-xl transition-colors duration-300 ${
                    userType === type ? 'text-primary' : 'text-gray-400 hover:text-gray-600'
                  }`}
               >
                 {type === 'merchant' ? <Store size={18} /> : <Truck size={18} />}
            ...
            <motion.div 
                ...
                initial={false}
                animate={{
                    left: userType === 'merchant' ? '6px' : '50%',
                    width: 'calc(50% - 9px)',
                    x: userType === 'delivery' ? 3 : 0
                }}
