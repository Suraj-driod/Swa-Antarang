...
import { subscribeToDeliveryUpdates, unsubscribeChannel } from '../../services/notificationService';
...
const LiveMap = ({ order }) => {
    if (!order) return null; // Safety check inside component too
        ...
        <div className="absolute inset-0 bg-[#fdf2f6]/30 overflow-hidden">
            ...
            <svg className="absolute inset-0 w-full h-full pointer-events-none">
                ...
                <motion.path
                    ...
                    animate={{ pathLength: (order.progress || 0) / 100 }}
...
const ChatOverlay = ({ target, onClose }) => {
    const [msgs, setMsgs] = useState([
        { id: 1, sender: 'them', text: `Hi, I am on my way with the ${target?.item || 'order'}.` },
    ]);
        ...
        >
            <div className="bg-[#59112e] p-4 flex justify-between items-center text-white">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h4 className="font-bold text-sm">{target?.driver?.name || 'Driver'}</h4>
            ...
            <div className="h-64 bg-[#fafafa] p-4 overflow-y-auto space-y-3">
                {msgs.map(m => (
                    <div key={m.id} className={`flex ${m.sender === 'me' ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[80%] px-3 py-2 rounded-xl text-xs ${m.sender === 'me' ? 'bg-[#59112e] text-white rounded-br-none' : 'bg-white border border-gray-200 text-gray-700 rounded-bl-none'
                            }`}>
                            {m.text}
                        </div>
                    </div>
...
const TrackingPanel = () => {
    ...
    // Subscribe to real-time delivery completions
    useEffect(() => {
        if (!user?.merchantProfileId) return;
        const channel = subscribeToDeliveryUpdates(user.merchantProfileId, (event) => {
            const notif = {
                id: Date.now(),
                orderId: event.orderId,
                deliveredAt: event.deliveredAt,
                orderName: event.order?.order_items?.[0]?.product_name || 'Order',
            ...
            // Also refresh the orders list to reflect updated status
            getOrdersByMerchant(user.merchantProfileId).then(data => {
                const mapped = data.filter(o => o.status !== 'cancelled').map(o => ({
                    ...
                    eta: o.status === 'delivered' ? 'Delivered' : 'Calculating...',
                    driver: { name: 'Assigning...', vehicle: '-', phone: '-', rating: 0 },
                    timeline: [
    ...
    useEffect(() => {
        ...
        getOrdersByMerchant(user.merchantProfileId).then(data => {
            const mapped = data.filter(o => o.status !== 'cancelled').map(o => ({
                ...
                status: STATUS_LABEL[o.status] || o.status,
                progress: STATUS_PROGRESS[o.status] || 0,
                eta: 'Calculating...', // Placeholder until real-time update
                driver: { name: 'Assigning...', vehicle: '-', phone: '-', rating: 0 },
                timeline: [
                    { time: new Date(o.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }), event: 'Order Placed', done: true },
                ],
    ...
    useEffect(() => {
        // FIX: Ensure we handle empty lists gracefully
        ...
        setShowChat(false);
    }, [activeTab, activeList.length]); // Added length dependency to trigger re-select on data load
                ...
                <div className="p-6 border-b border-[#f2d8e4] shrink-0">
                    ...
                    <div className="flex p-1 bg-[#fdf2f6] rounded-xl border border-[#f2d8e4]">
                        <button
                            ...
                            className={`flex-1 py-2.5 text-xs font-bold rounded-lg transition-all ${activeTab === 'sent' ? 'bg-white text-[#59112e] shadow-sm' : 'text-[#6b4c59] hover:text-[#59112e]'
                                }`}
                        >
                            Sent Orders
                        </button>
                        <button
                            ...
                            className={`flex-1 py-2.5 text-xs font-bold rounded-lg transition-all ${activeTab === 'received' ? 'bg-white text-[#59112e] shadow-sm' : 'text-[#6b4c59] hover:text-[#59112e]'
                                }`}
                        >
                            Incoming Stock
                        </button>
                    ...
                    {activeList.length === 0 ? (
                        ...
                        activeList.map((order) => (
                            <div
                                ...
                                className={`p-4 rounded-2xl border cursor-pointer transition-all hover:shadow-md ${selectedOrder?.id === order.id
            ...
            <div className="flex-1 relative bg-[#eef0f3] overflow-hidden flex flex-col h-full">
                ...
                {!selectedOrder ? (
                    <div className="flex flex-col items-center justify-center h-full text-gray-400">
                        ...
                        <p>Select an order to track details</p>
                    </div>
                ) : (
                    <>
                        {/* Live Map Area */}
                        <div className="flex-1 relative">
                            ...
                            <div className="absolute top-6 left-8 right-8 flex justify-between items-start z-10">
                                ...
                                <div className="bg-white/90 backdrop-blur-md px-4 py-2 rounded-xl shadow-sm border border-white/50">
                                    ...
                                    {/* FIX: Optional Chaining Here */}
                                    <p className="text-lg font-bold text-[#59112e]">{selectedOrder?.status || 'Unknown'}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="absolute bottom-8 left-8 right-8 z-20 flex gap-6 items-end pointer-events-none">
                            ...
                            >
                                <div className="flex items-center gap-4 mb-4">
                                    ...
                                    <div>
                                        <h3 className="font-bold text-[#2d0b16]">{selectedOrder?.driver?.name || 'Unassigned'}</h3>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    ...
                                    <button
                                        ...
                                        className={`flex items-center justify-center gap-2 py-2.5 rounded-xl text-white text-xs font-bold transition-colors shadow-lg ${showChat ? 'bg-[#450d24]' : 'bg-[#59112e] hover:bg-[#450d24]'
                                            }`}
                                    >
                                ...
                                <div className="flex justify-between items-center relative">
                                    ...
                                    {selectedOrder?.timeline?.map((step, idx) => (
                                        <div key={idx} className="flex flex-col items-center gap-3">
                                            <div className={`w-8 h-8 rounded-full border-4 flex items-center justify-center z-10 transition-colors duration-500 ${step.done
            ...
            <AnimatePresence>
                {notifications.map((notif, idx) => (
                    <motion.div
                        ...
                        className="fixed z-[2000] bg-white rounded-2xl shadow-2xl border border-emerald-200 p-5 flex items-start gap-4 max-w-sm"
                        style={{ top: `${24 + idx * 100}px`, right: '24px' }}
                    >
                        ...
                        <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1">
                                ...
                                <h4 className="font-bold text-slate-800 text-sm">Delivery Confirmed!</h4>
                            </div>
                            <p className="text-xs text-slate-500">{notif.orderName} — {notif.message}</p>
                            <p className="text-[10px] text-slate-400 mt-1">
                                {notif.deliveredAt ? new Date(notif.deliveredAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : 'Just now'}
                            </p>
                        </div>
                        <button
